...
import { BookOpen, ClipboardList, Trash2, Loader2, AlertCircle, X, GraduationCap } from 'lucide-react';
...
function AddApplicationModal({ targetId, onClose, onSuccess }) {
  ...
  // Carregar universidades ao abrir
  useEffect(() => {
  ...
  // Carregar faculdades quando universidade muda
  useEffect(() => {
    if (!selectedUniId) { setFaculties([]); setSelectedFacId(''); return; }
    setLoadingFac(true);
    setFaculties([]); setSelectedFacId(''); setCourses([]); setSelectedCourseId('');
    api.get(`/enrollment/faculties?universityId=${selectedUniId}`)
      .then((res) => setFaculties(res.data))
      .catch(() => {})
      .finally(() => setLoadingFac(false));
  }, [selectedUniId]);

  // Carregar cursos quando faculdade muda
  useEffect(() => {
    if (!selectedFacId) { setCourses([]); setSelectedCourseId(''); return; }
    setLoadingCourses(true);
    setCourses([]); setSelectedCourseId('');
    api.get(`/enrollment/courses?facultyId=${selectedFacId}`)
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body = {
        targetId,
        ...(useCustomUni    ? { customUniversity: customUniversity.trim() } : { universityId: selectedUniId }),
        ...(useCustomFac    ? { customFaculty: customFaculty.trim() }       : { facultyId: selectedFacId }),
        ...(useCustomCourse ? { customCourse: customCourse.trim() }         : { courseId: selectedCourseId }),
      };
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-5">
          ...
          {/* Universidade */}
          <div>
            <label className="block text-[#0A3956] font-semibold mb-2">Universidade</label>
            {useCustomUni ? (
              <input
                type="text"
              ...
              >
                <option value="">{loadingUnis ? 'A carregar...' : 'Selecciona uma universidade'}</option>
            ...
            <button
              onClick={() => {
                ...
                setCustomUniversity(''); setSelectedUniId('');
                setSelectedFacId(''); setSelectedCourseId('');
                setUseCustomFac(false); setUseCustomCourse(false);
                setCustomFaculty(''); setCustomCourse('');
              }}
              className="text-[#F69220] underline text-sm mt-2 hover:opacity-70 transition-opacity"
            >
              {useCustomUni ? '← Escolher da lista' : 'A universidade não está na lista? Escreve aqui'}
            </button>
          </div>

          {/* Faculdade */}
          <div>
            <label className="block text-[#0A3956] font-semibold mb-2">Faculdade</label>
            {useCustomFac ? (
              <input
                type="text"
                value={customFaculty}
              ...
              >
                <option value="">{loadingFac ? 'A carregar...' : 'Selecciona uma faculdade'}</option>
            ...
            <button
              onClick={() => {
                ...
                setCustomFaculty(''); setSelectedFacId('');
                setSelectedCourseId(''); setUseCustomCourse(false); setCustomCourse('');
              }}
              disabled={!uniOk}
            ...
            >
              {useCustomFac ? '← Escolher da lista' : 'A faculdade não está na lista? Escreve aqui'}
            </button>
          </div>

          {/* Curso */}
          <div>
            <label className="block text-[#0A3956] font-semibold mb-2">Curso</label>
            {useCustomCourse ? (
              <input
                type="text"
                value={customCourse}
              ...
              <select
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F69220] disabled:bg-[#F8F9FA] disabled:cursor-not-allowed bg-white"
              >
                <option value="">{loadingCourses ? 'A carregar...' : 'Selecciona um curso'}</option>
                {courses.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            )}
            <button
              onClick={() => { setUseCustomCourse(!useCustomCourse); setCustomCourse(''); setSelectedCourseId(''); }}
              disabled={!facOk}
              className="text-[#F69220] underline text-sm mt-2 hover:opacity-70 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {useCustomCourse ? '← Escolher da lista' : 'O curso não está na lista? Escreve aqui'}
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-3 mt-6">
          ...
          >
            {submitting ? <Loader2 className="animate-spin mx-auto" size={20} /> : 'Adicionar'}
          </button>
        </div>
      </div>
    </div>
...
export default function EnrollmentCard({ targets, loading, error, onRefresh }) {
  ...
  const hasCurrentYear = targets?.some((t) => t.year === currentYear && t.status !== 'CANCELLED');
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border-t-[3px] border-t-[#F69220] p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-[#0A3956] font-semibold text-base">Minha Inscrição</h2>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border-t-[3px] border-t-[#F69220] p-6 hover:shadow-md transition-shadow duration-200">
        ...
        {(!targets || targets.length === 0) && (
          <div className="bg-[#F8FAFC] rounded-lg p-8 text-center">
            ...
            <p className="text-[#6B7280] text-sm mb-4">Ainda não tem nenhuma inscrição.</p>
            <button
              onClick={() => navigate(ENROLLMENT_NEW_PATH)}
              className="px-6 py-2.5 bg-[#F69220] text-white rounded-lg font-semibold hover:bg-[#E08210] transition-colors text-sm"
            >
        ...
        <div className="space-y-4">
          {targets?.map((target) => {
            ...
            return (
              <div key={target.id} className="bg-[#F8FAFC] border border-[#E5E7EB] rounded-lg p-5">
                {/* Title row */}
                <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${STATUS_STYLES[target.status]}`}>
                ...
                <div className="mb-5">
                  ...
                  {target.applications?.length === 0 || !target.applications ? (
                    <div className="text-center py-6">
                      <GraduationCap className="mx-auto text-[#D1D5DB] mb-2" size={32} />
                      <p className="text-[#6B7280] text-xs">Nenhuma candidatura adicionada.</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {target.applications.map((app) => (
                        <div key={app.id} className="bg-white border border-[#E5E7EB] rounded-lg p-4">
                          <div className="flex items-start justify-between mb-2">
                            <h4 className="font-bold text-[#0A3956] text-sm">
                              {app.university?.name || app.customUniversity || 'Universidade'}
                            </h4>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <div className="text-[10px] uppercase tracking-wider text-[#6B7280] mb-1">FACULDADE</div>
                              <div className="text-[#1A1A2E] text-[13px]">
                                {app.faculty?.name || app.customFaculty || '—'}
                              </div>
                            </div>
                            <div>
                              <div className="text-[10px] uppercase tracking-wider text-[#6B7280] mb-1">CURSO</div>
                              <div className="text-[#1A1A2E] text-[13px]">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                    <div className="bg-white border border-[#E5E7EB] p-4 rounded-lg">
                      <div className="text-[11px] uppercase tracking-wider text-[#6B7280] mb-2">PREÇO BASE</div>
                      <div className="text-[#1A1A2E] font-medium text-sm">{formatCurrency(target.totalAmount)}</div>
                    </div>
                    <div className="bg-white border border-[#E5E7EB] p-4 rounded-lg">
                      <div className="text-[11px] uppercase tracking-wider text-[#6B7280] mb-2">BOLSA</div>
                      <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                        isFull ? 'bg-[#D4EDDA] text-[#155724]' : 'bg-[#FFF3CD] text-[#856404]'
                      }`}>
                        {SCHOLARSHIP_LABELS[target.scholarshipType]}
                      </span>
                    </div>
                    <div className="bg-white border border-[#E5E7EB] p-4 rounded-lg">
                      <div className="text-[11px] uppercase tracking-wider text-[#6B7280] mb-2">VALOR A PAGAR</div>
                      <div className="text-[#0A3956] font-bold text-lg">{formatCurrency(target.finalAmount)}</div>
                    </div>
                  </div>
                  <p className="text-xs text-[#6B7280] italic">
                    Valor calculado com base nas disciplinas inscritas e bolsa atribuída.
                  </p>
                </div>
              </div>
            );
